import {
	...
	var global = globalThis || window || this;

// 辅助解析函数 (用于兼容 && 语法)
global.__qjs_parseRule = function(rule) {
    if (!rule) return { selector: '', attr: '' };
    var parts = rule.split('&&');
    var attr = '';
    var last = parts[parts.length - 1];
    // 识别 Text, Html, href, src 等属性提取
...
// 1. 补齐 pdfh (用于解析并提取单段文本)
if (typeof global.pdfh === 'undefined') {
    global.pdfh = function (html, rule) {
        try {
            if (typeof cheerio === 'undefined') return "";
            var $ = cheerio.load(html);
            var p = global.__qjs_parseRule(rule);
...
// 2. 补齐 pd (用于解析并提取链接、图片地址等)
if (typeof global.pd === 'undefined') {
    global.pd = function (html, rule) {
        try {
            if (typeof cheerio === 'undefined') return "";
            var $ = cheerio.load(html);
            var p = global.__qjs_parseRule(rule);
...
// 3. 补齐 pdfa (用于解析并提取数组、列表)
if (typeof global.pdfa === 'undefined') {
    global.pdfa = function (html, rule) {
        try {
            if (typeof cheerio === 'undefined') return [];
            var $ = cheerio.load(html);
            var p = global.__qjs_parseRule(rule);
