...
var pdfh, pd, pdfa, local;

(function() {
    var g = typeof globalThis !== 'undefined' ? globalThis : (typeof window !== 'undefined' ? window : this);
    
    // 辅助解析函数 (处理 && 选择器)
    g.__qjs_parseRule = function(rule) {
        if (!rule) return { selector: '', attr: '' };
        var parts = rule.split('&&');
        var attr = '';
        var last = parts[parts.length - 1];
        if (['Text', 'Html', 'href', 'src', 'data-original', 'data-src', 'title'].includes(last) || last.indexOf('data-') === 0) {
            attr = parts.pop();
        }
        return { selector: parts.join(' ').trim(), attr: attr };
    };

    // 2. 赋予本地变量实际的解析逻辑
    pdfh = function (html, rule) {
        try {
            if (typeof cheerio === 'undefined') return "";
            var $ = cheerio.load(html);
            var p = g.__qjs_parseRule(rule);
            var node = p.selector ? $(p.selector) : $;
            if (p.attr === 'Text' || !p.attr) return node.text().trim();
            if (p.attr === 'Html') return node.html();
            return node.attr(p.attr) || "";
        } catch (e) { return ""; }
    };

    pd = function (html, rule) {
        try {
            if (typeof cheerio === 'undefined') return "";
            var $ = cheerio.load(html);
            var p = g.__qjs_parseRule(rule);
            var node = p.selector ? $(p.selector) : $;
            if (!p.attr) return node.attr('href') || node.attr('src') || node.attr('data-original') || "";
            if (p.attr === 'Text') return node.text().trim();
            return node.attr(p.attr) || "";
        } catch (e) { return ""; }
    };

    pdfa = function (html, rule) {
        try {
            if (typeof cheerio === 'undefined') return [];
            var $ = cheerio.load(html);
            var p = g.__qjs_parseRule(rule);
            ...
            $(p.selector).each(function() {
                result.push($(this).prop('outerHTML') || $(this).html());
            });
            return result;
        } catch (e) { return []; }
    };
    ...
    local = {
        get: function(rule, key) { return ""; },
        set: function(rule, key, value) { }
    };

    // 3. 同时挂载到全局对象，确保动态 eval() 也能访问到
    g.pdfh = pdfh;
    g.pd = pd;
    g.pdfa = pdfa;
    g.local = local;
...
const original_play = play;
const wrapped_play = async function(flag, id, flags) {
    let res = await original_play(flag, id, flags);
    try {
        // 将播放返回数据尝试解析为 JSON 对象
        let obj = typeof res === 'string' ? JSON.parse(res) : res;
        
        // 如果解析出合法的播放链接，且原规则没有写死 header 参数
        if (obj && obj.url && !obj.header) {
            // 自动注入当前站点自带的 rule.headers 或者是该站点的 rule.host 作为防盗链 Referer！
            obj.header = (typeof rule !== 'undefined' && rule.headers) || {
                "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15",
                "Referer": (typeof rule !== 'undefined' && rule.host) || ""
            };
        }
        return typeof res === 'string' ? JSON.stringify(obj) : obj;
    } catch(e) {
        return res; // 如果解析失败，原样返回防止崩溃
    }
};
